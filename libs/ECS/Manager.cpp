...
void ecs::Manager::registerEntity(ecs::Entity entity) {
    ...
    for (auto &sys : m_systems) {
        for (auto &neededComponents : sys->getNeededComponents()) {
            if (std::includes(m_entities[entity].begin(), m_entities[entity].end(), neededComponents.begin(),
                              neededComponents.end()) && !sys->hasEntity(entity))
        ...
        for (auto &it : elems) {
            if (m_componentManager.find(it) == m_componentManager.end()) {
                throw std::runtime_error(
                        std::string("This system need a ComponentManager that is not created: ") + std::to_string(it));
            }
        }
    }
