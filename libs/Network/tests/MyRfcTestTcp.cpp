...
#include <thread>
#include <Network/socket/include/TcpManager.hpp>
#include "catch.hpp"
#include "RFC/MyRfcClient.hpp"
#include "RFC/MyRfcServer.hpp"

void waitClientRequest(network::TcpManager &server)
{
	while (server.requestIsEmpty())
		;
}

void waitPingFromClient(network::MyRfcServer<network::TcpManager> &serverRfc)
{
...
TEST_CASE("Test ClientMyRfc Tcp template", "[Network]")
{
	...
	network::TcpManager server(5000);
	network::TcpManager client(5001);
	network::MyRfcClient<network::TcpManager> clientRfc(client);
	network::MyRfcServer<network::TcpManager> serverRfc(server);

	server.startAccept();
	REQUIRE(client.Connect(loc, 5000) == 0);
	SECTION("Test connection Server") {
		std::string login("Test");

		std::thread t(&network::MyRfcClient<network::TcpManager>::ConnectServer, &clientRfc, std::ref(loc), 5000, std::ref(login));
		waitClientRequest(server);
		auto res = serverRfc.getLastRequest();
		REQUIRE(res.data == "CONNECT Test\r\n");
		server.Write(res.sender, resList);
		t.join();

		std::thread t1(&network::MyRfcClient<network::TcpManager>::createGame, &clientRfc, std::ref(game), std::string(""));
		waitClientRequest(server);
		...
		std::thread t2(&network::MyRfcClient<network::TcpManager>::joinGame, &clientRfc, std::ref(game), std::string(""));
		waitClientRequest(server);
		res = serverRfc.getLastRequest();
		...
		int timeout {2};
		std::thread t3(&network::MyRfcClient<network::TcpManager>::isServerConnected, &clientRfc, timeout);
		waitClientRequest(server);
		res = serverRfc.getLastRequest();
...
TEST_CASE("Test ServerMyRfc tcp template", "[Network]")
{
	...
	SECTION("Test ServerRfc connection")
	{
		...
		t.join();
		std::thread t2(&network::MyRfcClient<network::TcpManager>::joinGame, &clientRfc, std::ref(gameLists[1]), std::string(""));
	...
	SECTION("Test serverRfc game send") {
		...
		std::thread t(
			&network::MyRfcClient<network::TcpManager>::FirstConnectToGame,
			&clientRfc, std::ref(loc), 5000, std::ref(login));
		while (serverRfc.requestIsEmpty());
		REQUIRE_NOTHROW(info = serverRfc.getLastRequest());
		REQUIRE_NOTHROW(
			serverRfc.entitiesInit(entityInit, info.sender));
		t.join();
		...
		REQUIRE(client.requestIsEmpty());
		int timeout {2};
		std::thread t3(&network::MyRfcClient<network::TcpManager>::isGameConnected, &clientRfc, timeout);
		waitClientRequest(server);
		auto res = serverRfc.getLastRequest();
		REQUIRE(res.data == "PING\r\n");
		REQUIRE(res.sender == 0);
		...
		REQUIRE(clientRfc.isGameConnected());
		t4.join();
	}
}
